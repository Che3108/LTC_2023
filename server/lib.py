...
def main_magic(Facility_df, Russia_df):
    ...
    for dates in list_date:
        ...
        df_timing = pd.DataFrame(data=timing_list)
        #print(df_timing)
        result[f"Duration_{dates}"] = df_timing
        ...
        duration['Террабайты, возможные для записи за интервал'] = duration['for_foto'].astype(float) * 4 * 0.00012207
