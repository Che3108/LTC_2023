...
Для запуска клиентской части потрбуется интерпритор python3 версии 3.8 и выше, а также установленная библиотека [requests](https://pypi.org/project/requests/)
Исполняемый файл находится в корне репозитория и назывется [client.py](). 
Для настройки клиентской части достаточно в исполняемом файле в переменной ARHIVE_path указать путь к архиву с исходными данными. Результатом работы клиентской части будет выведенный в консоль ответ от сервера в формате json 
